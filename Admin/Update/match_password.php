...
if(isset($_SESSION['special'])){
    if(isset($_SESSION['match'])){
        ...
        <h3 class=lev>Change Password</h3><br>
        <form method="POST" class='lev'> 
        <input class="form-control lev w-100" type="password" name="mtm" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" placeholder="New Password" required ><br>
        <input class="form-control lev w-100" type="password" name="szn" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" placeholder="Confirm Password" required><br>
        <button name="z" class= 'btn btn-primary'>Change Password</button>
        </form>
        </div>
        </center>
        <?php
        if(isset($_POST["z"])){
            ...
            if($new_pass==$retype_pass) {
                ...
                $typed_pass= "UPDATE admin_acess SET PASSWORD='$hash'  where name='$initial'";
                $z2=mysqli_query($con,$typed_pass);
                $_SESSION['uerror']="Password Changed";
